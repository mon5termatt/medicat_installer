...
for /f %%I in ('powershell -Command "$drive = Get-WmiObject Win32_LogicalDisk | Where-Object {$_.DeviceID -eq ('%drivepath%:' | ForEach-Object {$_ -replace '\\'})}; if ($drive.DriveType -eq 2) {exit 0} else {exit 1}"') do set "drivetype=%%I"
...
if %drivetype% equ 0 (
	echo Drive %drivepath% is a USB drive.
	goto usbyes
	) else (
	echo Drive %drivepath% is NOT a USB drive.
	goto usbno
	)
...
echo.[0m
echo.The installer will attempt to install to (%drivepath%). 
echo.PLEASE VERIFY THAT THIS IS THE CORRECT DRIVE
echo.Waiting for 15 seconds, GO VERIFY...
PING localhost -n 16 >NUL
Set /P drivepath=LAST CHANCE || Set drivepath=%drivepath%
IF "%drivepath%" == "C" GOTO IMPORTANTDRIVE
if "%format%" == "Yes" (goto formatdrive) else (goto updateventoy)
)
...
:CONFIRMUSB
echo.Installing to %drivepath%
echo.If this is correct just hit enter. CLOSE THE SCRIPT NOW IF THIS IS WRONG!!
Set /P drivepath=LAST CHANCE, VERIFY THIS IS THE USB: || Set drivepath=%drivepath%
IF "%drivepath%" == "C" GOTO IMPORTANTDRIVE
if "%format%" == "Yes" (goto formatdrive) else (goto updateventoy)
