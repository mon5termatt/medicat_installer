# Test script for installing/uninstalling PS7Zip module
# Usage: .\Test-PS7Zip-Install.ps1 [install|uninstall|status]
#   install   - Install PS7Zip module
#   uninstall - Uninstall PS7Zip module  
...
Write-Host "=== PS7Zip Module Installation Tester ===" -ForegroundColor Cyan
...
# Function to find module in gallery
function Test-ModuleInGallery {
    ...
    try {
        ...
        Write-Host "[ERROR] Module not found in gallery: $($_.Exception.Message)" -ForegroundColor Red
        return $false
    }
}
...
$wasInstalled = Test-ModuleInstalled -ModuleName "PS7Zip"
Write-Host ""
...
switch ($Action.ToLower()) {
    "status" {
        ...
        if ($wasInstalled) {
            ...
            $module = Get-Module -ListAvailable -Name "PS7Zip" | Select-Object -First 1
            Write-Host "Name: $($module.Name)" -ForegroundColor White
            Write-Host "Version: $($module.Version)" -ForegroundColor White
            ...
            try {
                Import-Module PS7Zip -Force -ErrorAction Stop
                $imported = Get-Module PS7Zip
                Write-Host ""
                ...
                $expandCmd = Get-Command Expand-7Zip -ErrorAction SilentlyContinue
                if ($expandCmd) {
                    Write-Host ""
                    Write-Host "Expand-7Zip cmdlet found" -ForegroundColor Green
                    $params = $expandCmd.Parameters.Keys | Where-Object { $_ -notlike 'Verbose*' -and $_ -notlike 'Debug*' -and $_ -notlike 'Error*' -and $_ -notlike 'Warning*' -and $_ -notlike 'Information*' -and $_ -notlike 'Out*' -and $_ -notlike 'Pipeline*' -and $_ -notlike 'WhatIf' -and $_ -notlike 'Confirm' }
                    Write-Host "  Parameters: $($params -join ', ')" -ForegroundColor Gray
                }
                
                Remove-Module PS7Zip -Force -ErrorAction SilentlyContinue
            } catch {
    ...
    "uninstall" {
        Write-Host "=== Uninstall PS7Zip ===" -ForegroundColor Cyan
        
        if ($wasInstalled) {
            Write-Host "Uninstalling PS7Zip module..." -ForegroundColor Yellow
            try {
                Uninstall-Module -Name "PS7Zip" -AllVersions -Force -ErrorAction Stop
                Write-Host "[OK] Module uninstalled successfully" -ForegroundColor Green
                
                # Verify uninstall
                Start-Sleep -Seconds 1
                $stillInstalled = Test-ModuleInstalled -ModuleName "PS7Zip"
                if ($stillInstalled) {
    ...
    "install" {
        Write-Host "=== Install PS7Zip ===" -ForegroundColor Cyan
        
        # Show PSGallery status
        Show-PSGalleryStatus
        
        # Check if module exists in gallery
        $inGallery = Test-ModuleInGallery -ModuleName "PS7Zip"
        Write-Host ""
        
        if (-not $inGallery) {
            Write-Host "[ERROR] Cannot proceed - PS7Zip not found in PowerShell Gallery" -ForegroundColor Red
            exit 1
        }
        
        if ($wasInstalled) {
            Write-Host "[INFO] Module already installed (v$((Get-Module -ListAvailable -Name PS7Zip | Select-Object -First 1).Version))" -ForegroundColor Yellow
        ...
        Write-Host "Installing PS7Zip module..." -ForegroundColor Yellow
        ...
        try {
            ...
            # Install module
            Write-Host "Downloading and installing PS7Zip..." -ForegroundColor Yellow
            Install-Module -Name "PS7Zip" -Force -Scope CurrentUser -ErrorAction Stop
            Write-Host "[OK] Module installed successfully" -ForegroundColor Green
            
            # Verify install
            Start-Sleep -Seconds 1
            $nowInstalled = Test-ModuleInstalled -ModuleName "PS7Zip"
            if ($nowInstalled) {
                ...
                try {
                    ...
                    # Test Expand-7Zip if available
                    $expandCmd = Get-Command Expand-7Zip -ErrorAction SilentlyContinue
                    if ($expandCmd) {
                        Write-Host "[OK] Expand-7Zip cmdlet available" -ForegroundColor Green
                        $params = $expandCmd.Parameters.Keys | Where-Object { $_ -notlike 'Verbose*' -and $_ -notlike 'Debug*' -and $_ -notlike 'Error*' -and $_ -notlike 'Warning*' -and $_ -notlike 'Information*' -and $_ -notlike 'Out*' -and $_ -notlike 'Pipeline*' -and $_ -notlike 'WhatIf' -and $_ -notlike 'Confirm' }
                        Write-Host "     Parameters: $($params -join ', ')" -ForegroundColor Gray
                    }
